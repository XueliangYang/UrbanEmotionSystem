...
sys.path.append(os.path.join(os.getcwd(), 'Usage_v6_Brooklyn', 'models'))
...
try:
    ...
    print("Make sure you're running this script from the EmoMA-Net directory.")
    sys.exit(1)
...
def process_participant(folder, model, device):
    """
    Process a single participant's data (prediction only).
    ...
    try:
        ...
        # Look for the prepared file without DateTime
        input_file = os.path.join(folder, f'P{participant_id}_Prepared.csv')
        
        if not os.path.exists(input_file):
            print(f"Warning: Could not find prepared file {input_file}")
            print(f"Make sure to run prepare_data.py first to create this file.")
            return False
        ...
        df_model_ready = prepare_data_for_model(df, input_file)
...
def main():
    ...
    model_path = os.path.join(os.getcwd(), 'Usage_v6_Brooklyn', 'models', 'best_model_v6.pth')
